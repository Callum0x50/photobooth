...
const int camera_shutter = 13;
const int builtin_button_LED = 2;
...
void setup() {
  ...
  pinMode(builtin_button_LED, OUTPUT);
}
  
void loop() {
  digitalWrite(builtin_button_LED, HIGH);
  
  if(digitalRead(6) == HIGH) {
      digitalWrite(builtin_button_LED, LOW);

      for (int i = 0; i < 4; i++) {
        strip.setBrightness(40);
    
        colorWipe(strip.Color(0, 255, 0), 50); // Red
        colorWipe(strip.Color(62, 255, 0), 50); // Yellow
